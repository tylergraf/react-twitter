...
router.all('/proxy/*', createTwitterClient, proxy);

function proxy(req, res, next) {
  var path = req.path.split('/proxy/')[1],
      options = req.query || {};
  ...
  req.twit[req.method.toLowerCase()](path, options)
    .then(
      data => {
        res.status(data.resp.statusCode).json(data.data)},
      err => next(err)
    );
}
